...
export async function POST(request: Request) {
  ...
  if (isRateLimited("post", userId)) {
    return NextResponse.json({ error: rateLimitMessage("post") }, { status: 429 });
  }
  ...
  if (image instanceof File && image.size > 0) {
    ...
    if (ext === "gif") {
      // animasyon bozulmasın diye gif'e dokunma
      const name = `${crypto.randomUUID()}.gif`;
      fs.writeFileSync(path.join(UPLOAD_DIR, name), buffer);
      imagePath = name;
    } else {
      // küçült + webp'e çevir: disk ve bant genişliği tasarrufu
      ...
      const resized = await sharp(buffer)
        ...
        .webp({ quality: 82 })
        .toBuffer();
      fs.writeFileSync(path.join(UPLOAD_DIR, name), resized);
      imagePath = name;
    }
  }
